...
class TrimNet(torch.nn.Module):
    def __init__(self, in_dim, edge_in_dim, hidden_dim=64, depth=3, heads=4, dropout=0.1, outdim=86,kge_dim=128, rel_total=86):
        ...
        self.mlp = nn.ModuleList([nn.Linear(hidden_dim*4, 256),
                                  nn.ELU(),
                                  nn.Dropout(p=0.1),
                                  nn.Linear(256, 128),
                                  nn.ELU(),
    ...
    def get_weight(self, triples,zhongzi,epoch,save_path):
        #print(triples)
        h_data, _, _= triples
        ...
        repr_h=xall.view((1700,-1))
        checkpoint_path = os.path.join(save_path, f'drug_emb_trimnet'+str(zhongzi)+'.npy')
        np.save(checkpoint_path,repr_h.cpu())
