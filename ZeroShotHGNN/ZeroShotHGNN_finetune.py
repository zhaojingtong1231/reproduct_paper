...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default="rgat",
        choices=["rgcn", "rsage",  "rgat"],  # 指定合法的候选项
        help="Choose the model type: rgcn, rsage or rgat."
    )
    ...
    model_save_path = os.path.join(model_save_path,
                             f"model_{model}_lr{lr}_batch{batch_size}_epochs{epochs}_hidden{hidden_dim}_time{time_str}_seed{seed}")
    ...
    pretrain_model_path = '/data/zhaojingtong/PrimeKG/model/model22_2000.pth'
