...
def preprocess_kg(path, split, test_size=0.05, one_hop=False, mask_ratio=0.1):
    if split in ['cell_proliferation', 'mental_health', 'cardiovascular', 'anemia', 'adrenal_gland', 'autoimmune',
        ...
        df = df[['x_type', 'x_id', 'relation', 'y_type', 'y_id','x_name','y_name']]
...
def evaluate_gnnexplainer(model, G, g_valid_pos, g_valid_neg, only_relation, epoch, etypes_train, penalty_scaling,
                          device, mode='validation', weight_bias_track=False, wandb=None):
    loss_fct = nn.MSELoss()
    model.eval()
    G = G.to(device)
    with torch.no_grad():
        original_predictions_pos, original_predictions_neg, _, _ = model.gnnexplainer_forward(G, g_valid_pos,
                                                                                              g_valid_neg,
        ...
        updated_predictions_pos, updated_predictions_neg, penalty, num_masked = model.gnnexplainer_forward(G,
                                                                                                           g_valid_pos,
                                                                                                           g_valid_neg,
                                                                                                           graphmask_mode=True,
                                                                                                           only_relation=only_relation)
...
def evaluate_graphmask(model, G, g_valid_pos, g_valid_neg, only_relation, epoch, etypes_train, allowance,
    ...
    with torch.no_grad():
        ...
        original_predictions_pos, original_predictions_neg, _, _ = model.graphmask_forward(G, g_valid_pos, g_valid_neg,
                                                                                           graphmask_mode=False,
                                                                                           only_relation=only_relation,
                                                                                           no_base=no_base)
        ...
        updated_predictions_pos, updated_predictions_neg, penalty, num_masked = model.graphmask_forward(G, g_valid_pos,
                                                                                                        g_valid_neg,
                                                                                                        graphmask_mode=True,
                                                                                                        only_relation=only_relation,
                                                                                                        no_base=no_base)
...
def evaluate_ib(model, G, g_valid_pos, g_valid_neg, only_relation, epoch, etypes_train, allowance, penalty_scaling,
                device, mode='validation', weight_bias_track=False, wandb=None, no_base=False):
    model.eval()
    G = G.to(device)
    with torch.no_grad():
        loss_fct = nn.MSELoss()
        ...
        original_predictions_pos, original_predictions_neg, _, _ = model.ib_forward(G, g_valid_pos, g_valid_neg,
                                                                                    graphmask_mode=False,
                                                                                    only_relation=only_relation,
                                                                                    no_base=no_base)
        ...
        updated_predictions_pos, updated_predictions_neg, penalty, num_masked = model.ib_forward(G, g_valid_pos,
                                                                                                 g_valid_neg,
                                                                                                 graphmask_mode=True,
                                                                                                 only_relation=only_relation,
                                                                                                 no_base=no_base)
...
def evaluate_mb(model, g_pos, g_neg, G, dd_etypes, device, return_embed=False, mode='valid'):
    model.eval()
    # model = model.to('cpu')
    pred_score_pos, pred_score_neg, pos_score, neg_score = model.forward_minibatch(g_pos.to(device), g_neg.to(device),
                                                                                   [G.to(device), G.to(device)],
                                                                                   G.to(device), mode=mode,
                                                                                   pretrain_mode=False)
...
def process_df(df_train, edge_dict):
    ...
    df_train = df_train[['x_type', 'x_idx', 'relation_idx', 'y_type', 'y_idx', 'degree', 'label']].rename(
        columns={'x_type': 'head_type',
                 'x_idx': 'head',
...
def initialize_node_embedding(g, n_inp,pickle_path):
    with open(pickle_path+'/drug_embeddings.pkl', 'rb') as f:
        drug_embeddings_dict = pickle.load(f)
    with open(pickle_path+'/disease_embeddings.pkl', 'rb') as f:
    ...
    for ntype in g.ntypes:
        ...
        if ntype=='drug':
            for i in range(g.number_of_nodes(ntype)):
                drug_embeddings.append(torch.tensor(drug_embeddings_dict[i],dtype=torch.float32))
            g.nodes[ntype].data['inp'] = nn.Parameter(torch.stack(drug_embeddings), requires_grad=False)
        elif ntype=='disease':
            for i in range(g.number_of_nodes(ntype)):
                disease_embeddings.append(torch.tensor(disease_embeddings_dict[i],dtype=torch.float32))
            g.nodes[ntype].data['inp'] = nn.Parameter(torch.stack(disease_embeddings), requires_grad=False)
        else:
...
def disease_centric_evaluation(df, df_train, df_valid, df_test, data_path, G, model, device, disease_ids=None,
    ...
    G = G.to(device)
    model = model.eval()
    from sklearn.metrics import accuracy_score, roc_curve, average_precision_score, recall_score, confusion_matrix, \
        classification_report, roc_auc_score, f1_score, auc, precision_recall_curve
    ...
    def calculate_metrics(rel, preds_all, labels_all, mode='drug', subset_mode=True):
        ...
        for i, j in k_num.items():
            AP[i], MRR[i], Recall[i], Recall_Random[i], Enrichment[i], not_in_ranked_list[i], in_ranked_list[
                i] = {}, {}, {}, {}, {}, {}, {}
                ...
                if len(pos_idx) == 0:
                    ...
                    if simulate_random:
                        Recall_Random[i][entity_id] = np.mean(
                            [len(np.intersect1d(sim_trial[:k_num[i]], pos_idx)) / len(pos_idx) for sim_trial in
                             ranked_list_random])
                    else:
    ...
    def summary(result, rel_type, mode='drug', show_plot=True, verbose=True):
        ...
        if show_plot:
            ...
            import matplotlib.pyplot as plt
            sns.scatterplot(list(range(len(result['Recall@5%']))), list(result['# of Pos'].values())).set_title(
                "#pos scatter plot")
            plt.show()

            for i in ['Recall@1%', 'Recall@5%', 'Recall@10%', 'Recall@10', 'Recall@50', 'Recall@100', 'AUROC', 'AUPRC',
                      'MRR@10', 'MRR@50', 'MRR@100', 'AP@10', 'AP@50', 'AP@100']:
                sns.distplot(list(result[i].values())).set_title(i + " distribution")
                plt.show()

            preds_ = np.concatenate([np.array(list(j.values())) for i, j in results['Prediction'].items()]).reshape(
                -1, )
    ...
    def get_scores_disease(rel, disease_ids):
        ...
        for disease_id in tqdm(disease_ids):
            ...
            g_eval = dgl.heterograph(out, num_nodes_dict={ntype: G.number_of_nodes(ntype) for ntype in G.ntypes}).to(
                device)
            ...
            preds_contra[idx2id_disease[disease_id]] = {idx2id_drug[i]: pred[idx] for idx, i in
                                                        enumerate(g_eval.edges()[1].detach().cpu().numpy())}
    ...
    if disease_ids is None:
        ...
        for rel_type in disease_rel_types:
            ...
            out_dict_mean, out_dict_std = summary(results, rel_type, mode='disease', show_plot=show_plot,
                                                  verbose=verbose)
...
def drop_edge(blocks):
    new_blocks = []
    for block in blocks:
        transform = DropEdge(p=0.2)
        g = dgl.block_to_graph(block)
        drop_edge_g = transform(g)
        block = dgl.to_block(drop_edge_g)
