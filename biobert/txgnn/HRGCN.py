...
class HeteroRGCNLayer(nn.Module):
    ...
    def forward(self, G, feat_dict):
        ...
        for srctype, etype, dsttype in etypes_all:
            # 直接将特征存储到节点数据中，无需变换
            G.nodes[srctype].data['feat_%s' % etype] = feat_dict[srctype]

            # 创建一个虚拟的消息传递函数，消息内容不做任何操作
            funcs[etype] = (fn.copy_u('feat_%s' % etype, 'm'), fn.mean('m', 'h'))

        # 执行消息传递（不过不会实际做任何特征变换，只是保持接口一致）
        G.multi_update_all(funcs, 'sum')

        # 返回目标节点的特征，保持输入输出格式一致
