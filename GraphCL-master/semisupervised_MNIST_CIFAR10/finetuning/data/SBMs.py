...
class SBMsDatasetDGL(torch.utils.data.Dataset):
    ...
    def __init__(self, name):
        ...
        """
        start = time.time()
        print("[I] Loading small_data ...")
        self.name = name
        data_dir = 'small_data/SBMs'
...
class SBMsDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name):
        ...
        percent = 0.1
        self.train = random_split(self.train_all, percent)
...
'''
def random_split(dataset, percent):
    num = len(dataset)
    split_num = int(num * percent)
    all_index = [i for i in range(num)]
    choice_index = random.sample(all_index, split_num)
    graphs_list = []
    labels_list = []
    for i in choice_index:
        graphs_list.append(dataset[i][0])
        labels_list.append(dataset[i][1])
    return DGLFormDataset(graphs_list, labels_list)