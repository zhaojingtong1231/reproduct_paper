...
def main():    
    ...
    parser.add_argument('--load_model',  action='store_true',   
                         default=False,   help='run type')
    parser.add_argument('--head', action='store_true', default=False,
        help="use head or not")  
    parser.add_argument('--aug', type=int, default=0,
        help="Please give a value for gpu id")  
    parser.add_argument('--config', type=int, default=0 ,
        help="Please give a config.json file with training/model/small_data/param details")
    ...
    print('-'*100)
    print("Train Average: [{:.2f}], Std: [{:.2f}]".format(np.mean(np.array(train_acc_list)), np.std(train_acc_list)))
    print("Test  Average: [{:.2f}], Std: [{:.2f}]".format(np.mean(np.array(test_acc_list)), np.std(test_acc_list)))
    print('-'*100)
