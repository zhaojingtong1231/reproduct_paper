...
class ExtractSubstructureContextPair:
    def __init__(self, k, l1, l2):
        """
        Randomly selects a node from the small_data object, and adds attributes
    ...
    def __call__(self, data, root_idx=None):
        ...
        :param data: pytorch geometric small_data object
        :param root_idx: If None, then randomly samples an atom idx.
        Otherwise sets atom idx of root (for debugging only)
        :return: None. Creates new attributes in original small_data object:
        small_data.center_substruct_idx
        small_data.x_substruct
        small_data.edge_attr_substruct
        small_data.edge_index_substruct
        small_data.x_context
        small_data.edge_attr_context
        small_data.edge_index_context
        small_data.overlap_context_substruct_idx
        """
        num_atoms = data.x.size()[0]
        if root_idx == None:
            root_idx = random.sample(range(num_atoms), 1)[0]

        G = graph_data_obj_to_nx_simple(data)  # same ordering as input small_data obj
        ...
        if len(substruct_node_idxes) > 0:
            ...
            # to reset node idx to 0 -> num_nodes - 1, otherwise small_data obj does not
            # make sense, since the node indices in small_data obj must start at 0
        ...
        if len(context_node_idxes) > 0:
            ...
            # reset node idx to 0 -> num_nodes - 1, otherwise small_data obj does not
            # make sense, since the node indices in small_data obj must start at 0
        ...
        # ### For debugging ###
        # if len(substruct_node_idxes) > 0:
        #     substruct_mol = graph_data_obj_to_mol_simple(small_data.x_substruct,
        #                                                  small_data.edge_index_substruct,
        #                                                  small_data.edge_attr_substruct)
        #     print(AllChem.MolToSmiles(substruct_mol))
        # if len(context_node_idxes) > 0:
        #     context_mol = graph_data_obj_to_mol_simple(small_data.x_context,
        #                                                small_data.edge_index_context,
        #                                                small_data.edge_attr_context)
...
class MaskAtom:
    ...
    def __call__(self, data, masked_atom_indices=None):
        ...
        :param data: pytorch geometric small_data object. Assume that the edge
        ordering is the default pytorch geometric ordering, where the two
        directions of a single edge occur in pairs.
        Eg. small_data.edge_index = tensor([[0, 1, 1, 2, 2, 3],
                                     [1, 0, 2, 1, 3, 2]])
        ...
        * mask rate number of atom indices
        ...
        debugging only)
        :return: None, Creates new attributes in original small_data object:
        small_data.mask_node_idx
        small_data.mask_node_label
        small_data.mask_edge_idx
        small_data.mask_edge_label
        """
...
if __name__ == "__main__":
    ...
    small_data = mol_to_graph_data_obj_simple(m)
    root_idx = 13

    # 0 hops: no substructure or context. We just test the absence of x attr
    transform = ExtractSubstructureContextPair(0, 0, 0)
    transform(small_data, root_idx)
    assert not hasattr(small_data, 'x_substruct')
    assert not hasattr(small_data, 'x_context')
    ...
    for i in range(len(m.GetAtoms())):
        ...
        transform(small_data, root_idx)
        if hasattr(small_data, 'x_substruct'):
            n_substruct_atoms = small_data.x_substruct.size()[0]
        else:
            n_substruct_atoms = 0
        print('n_substruct_atoms: {}'.format(n_substruct_atoms))
        if hasattr(small_data, 'x_context'):
            n_context_atoms = small_data.x_context.size()[0]
        else:
            n_context_atoms = 0
    ...
    transform(small_data, root_idx)
    assert hasattr(small_data, 'center_substruct_idx')
    ...
    transform(small_data, masked_atom_indices)
    assert small_data.mask_node_label.size() == torch.Size(
        (len(masked_atom_indices), 2))
    assert not hasattr(small_data, 'mask_edge_label')
    # check that the correct rows in x have been modified to be mask atom type
    assert (small_data.x[masked_atom_indices] == torch.tensor(([num_atom_type,
                                                          0]))).all()
    assert (small_data.mask_node_label == original_data.x[masked_atom_indices]).all()
    ...
    connected_edge_indices = []
    for i in range(small_data.edge_index.size()[1]):
        if tuple(small_data.edge_index.numpy().T[i].tolist()) in rdkit_bonds:
            connected_edge_indices.append(i)
    assert (small_data.edge_attr[connected_edge_indices] ==
            torch.tensor(([num_edge_type, 0]))).all()
    assert (small_data.mask_edge_label == original_data.edge_attr[
        connected_edge_indices[::2]]).all() # small_data.mask_edge_label contains
    # the unique edges (ignoring direction). The small_data obj has edge ordering
