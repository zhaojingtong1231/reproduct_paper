...
class ImageDataset(InMemoryDataset):
    def __init__(self,
                 ...
                 pre_filter=None,
                 coord=False,
                 processed_file_prefix='small_data'):
        assert name in ['MNIST', 'CIFAR10'], "Unsupported small_data name %s" % name
        self.name = name
        self.coord = coord
    ...
    def download(self):
        ...
        if self.name == 'CIFAR10':
            ...
            raise ValueError("Unknown small_data name {}".format(self.name))
