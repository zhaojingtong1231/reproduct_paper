    ...
    def pretrain(self, n_epoch = 1, learning_rate = 1e-3, batch_size = 1024, train_print_per_n = 20, save_model_path='./' ,sweep_wandb = None):
        ...
        for epoch in range(n_epoch):
            ...
            for step, (nodes, pos_g, neg_g, blocks) in enumerate(dataloader):
                ...
                pred_score_pos, pred_score_neg, pos_score, neg_score,dgi_loss = self.model.forward_minibatch(pos_g, neg_g, blocks, self.G, mode = 'train', pretrain_mode = True)
                ...
                if (step) % train_print_per_n == 0:
                    ...
                    tauroc_rel, tauprc_rel, tmicro_auroc, tmicro_auprc, tmacro_auroc, tmacro_auprc = get_all_metrics_fb(pred_score_pos, pred_score_neg, scores.reshape(-1,).detach().cpu().numpy(), labels, self.G, True)
