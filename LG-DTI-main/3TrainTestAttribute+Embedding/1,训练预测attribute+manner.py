...
testSample = []
ReadMyCsv2(testSample,"testSample.csv")
tprs = []
aucs = []
mean_fpr = np.linspace(0, 1, 1000)
i = 0
colorlist = ['red', 'gold', 'purple', 'green', 'blue','orange','pink','gray','lightskyblue','brown','black']
RealAndPrediction = []
AllResult = []
...
while counter < 5:
    ...
    while counter1 < len(PositiveSample):
        FeaturePair = []
        FeaturePair.extend(AllNodeFeatureNum[int(PositiveSample[counter1][0])])
        FeaturePair.extend(AllNodeFeatureNum[int(PositiveSample[counter1][1])])
    ...
    testSampleFeature = []
    counter3 = 0
    while counter3 < len(testSample):
        FeaturePair = []
        FeaturePair.extend(AllNodeFeatureNum[testSample[counter3][0]])
        FeaturePair.extend(AllNodeFeatureNum[testSample[counter3][1]])
    ...
    model1.fit(np.array(TrainFeature), np.array(TrainLabel))
    print(TestFeature)
    y_score0 = model1.predict(TestFeature)
    y_score1 = model1.predict_proba(TestFeature)
    y_score0_0 = model1.predict(testSampleFeature)
    y_score1_1 = model1.predict_proba(testSampleFeature)
    StorFile(y_score1,"result" + str(counter) + ".csv")
    #StorFile(y_score1_1, "result" + str(counter) + ".csv")
...
#StorFile(MyNew, '五折的表-attribute+manner3.csv')
...
#MyEnlarge(0, 0.7, 0.25, 0.25, 0.5, 0, 2, mean_fpr, mean_tpr, 2, colorlist[10])
...
# 保存图片
plt.savefig('5fold-attribute+manner3.svg')
plt.savefig('5fold-attribute+manner3.tif')
plt.show()