...
class TUDataset_aug(InMemoryDataset):
    ...
    Args:
        ...
        transform (callable, optional): A function/transform that takes in an
            :obj:`torch_geometric.small_data.Data` object and returns a transformed
            version. The small_data object will be transformed before every access.
            (default: :obj:`None`)
        pre_transform (callable, optional): A function/transform that takes in
            an :obj:`torch_geometric.small_data.Data` object and returns a
            transformed version. The small_data object will be transformed before
            being saved to disk. (default: :obj:`None`)
        pre_filter (callable, optional): A function that takes in an
            :obj:`torch_geometric.small_data.Data` object and returns a boolean
            value, indicating whether the small_data object should be included in the
    ...
    def get(self, idx):
        ...
        """
        edge_index = small_data.edge_index
        node_num = small_data.x.size()[0]
        edge_num = small_data.edge_index.size()[1]
        small_data.edge_index = torch.tensor([[edge_index[0, n], edge_index[1, n]] for n in range(edge_num) if edge_index[0, n] < node_num and edge_index[1, n] < node_num] + [[n, n] for n in range(node_num)], dtype=torch.int64).t()
        """
        ...
        # if self.aug == 'dnodes':
        #     data_aug = drop_nodes(deepcopy(small_data))
        # elif self.aug == 'pedges':
        #     data_aug = permute_edges(deepcopy(small_data))
        # elif self.aug == 'subgraph':
        #     data_aug = subgraph(deepcopy(small_data))
        # elif self.aug == 'mask_nodes':
        #     data_aug = mask_nodes(deepcopy(small_data))
        # elif self.aug == 'none':
        #     data_aug = deepcopy(small_data)
        #     data_aug.x = torch.ones((small_data.edge_index.max()+1, 1))
...
def drop_nodes(data):
    ...
    # small_data.x = small_data.x[idx_nondrop]
    ...
    # small_data.edge_index = torch.tensor(edge_index).transpose_(0, 1)
