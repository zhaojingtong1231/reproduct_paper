...
def nx_to_graph_data_obj(g, center_id, allowable_features_downstream=None,
    ...
    # between nx node ids and small_data obj node indices
    ...
    for node_1, node_2, attr_dict in g.edges(data=True):
        ...
        # convert nx node ids to small_data obj node index
    ...
    # small_data.edge_index: Graph connectivity in COO format with shape [2, num_edges]
    edge_index = torch.tensor(np.array(edges_list).T, dtype=torch.long)

    # small_data.edge_attr: Edge feature matrix with shape [num_edges, num_edge_features]
    ...
    if node_id_to_go_labels:  # supervised case with go node labels
        ...
        # n_downstream_go_classes tensor for the center node. 0 is no small_data
...
def graph_data_obj_to_nx(data):
    """
    Converts pytorch geometric Data obj to network x small_data object.
    ...
    # nx.set_node_attributes(G, {small_data.center_node_idx.item(): True}, 'is_centre')
...
class BioDataset_aug(InMemoryDataset):
    def __init__(self,
        ...
        :param root: the small_data directory that contains a raw and processed dir
        :param data_type: either supervised or unsupervised
        :param empty: if True, then will not load any small_data obj. For
    ...
    def get(self, idx):
        ...
        for key in self.data.keys:
            ...
            # s[small_data.cat_dim(key, item)] = slice(slices[idx],
            #                                         slices[idx + 1])
