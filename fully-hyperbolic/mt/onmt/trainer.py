...
class Trainer(object):
    ...
    def __init__(self, model, train_loss, valid_loss, optim,
                 ...
                 source_noise=None, max_grad_norm=1,):
        # Basic attributes.
        self.model = model
    ...
    def validate(self, valid_iter, moving_average=None):
        ...
        with torch.no_grad():
            ...
            for batch in valid_iter:
                ...
                # Compute loss.
                outputs = valid_model.decoder.cal_dist(outputs)
    ...
    def _gradient_accumulation(self, true_batches, normalization, total_stats,
        ...
        for k, batch in enumerate(true_batches):
            ...
            for j in range(0, target_size - 1, trunc_size):
                ...
                #                                     with_align=self.with_align, init=True)
                #         return
                outputs, attns = self.model(src, tgt, src_lengths, bptt=bptt,
                                            with_align=self.with_align)
                bptt = True
                # 3. Compute loss.
                # if self.opt.decoder == 'htransformer':
                outputs = self.model.decoder.cal_dist(outputs)
                outputs = outputs
                addition_loss = 0
                # addition_loss = 1e-6 * self.model.decoder.decision_norm()
                try:
