...
def batch_producer(generator_to_serve, queues, semaphore, opt):
    ...
    for device_id, q in cycle(enumerate(queues)):
        ...
        if isinstance(b.src, tuple):
            b.src = tuple([_.to(torch.device(device_id), non_blocking=True)
                           for _ in b.src])
        else:
            b.src = b.src.to(torch.device(device_id), non_blocking=True)
        b.tgt = b.tgt.to(torch.device(device_id), non_blocking=True)
        b.indices = b.indices.to(torch.device(device_id), non_blocking=True)
        b.alignment = b.alignment.to(torch.device(device_id), non_blocking=True) \
            if hasattr(b, 'alignment') else None
        b.src_map = b.src_map.to(torch.device(device_id), non_blocking=True) \
            if hasattr(b, 'src_map') else None
        b.align = b.align.to(torch.device(device_id), non_blocking=True) \
            if hasattr(b, 'align') else None
        b.corpus_id = b.corpus_id.to(torch.device(device_id), non_blocking=True) \
...
def _get_parser():
    parser = ArgumentParser(description='train_HAN.py')
