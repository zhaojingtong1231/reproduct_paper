...
###### Dataset
df_ddi_train = pd.read_csv('small_data/' + str(zhongzi) + '/ddi_training1.csv')
df_ddi_val = pd.read_csv('small_data/' + str(zhongzi) + '/ddi_validation1.csv')
df_ddi_test = pd.read_csv('small_data/' + str(zhongzi) + '/ddi_test1.csv')
...
train_data_loader = DrugDataLoader(train_data, batch_size=batch_size, shuffle=True, drop_last=True,num_workers=32)
val_data_loader = DrugDataLoader(val_data, batch_size=batch_size, drop_last=True,num_workers=32)
test_data_loader = DrugDataLoader(test_data, batch_size=batch_size, drop_last=False,num_workers=32)
druglist = DrugDataLoader(druglist)


def compute_weigth(batch, device, epoch,training=True):
    '''
    ...
    for batch in druglist:
        ...
        h_weight = model.get_weight(pos_tri, zhongzi,epoch)
        break
    ...
    for i in range(1, n_epochs + 1):
        ...
        with torch.no_grad():
            ...
            if test_acc >= test_acc_history:
                ...
                compute_weigth(druglist, device, i)
...
save_path = os.path.join(save_dir,
                             f"zhongzi{zhongzi}_time_{time_str}")
