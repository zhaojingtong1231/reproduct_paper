...
def main():
    ...
    params = dict(
        model='MPNN-DDI',
        data_root='small_data/preprocessed/',
        save_dir='save',
        dataset='drugbank',
        epochs=args.epochs,
        fold=args.fold,
    ...
    kge_dim = params.get('kge_dim')
    rel_total = params.get('rel_total')
    hidden_dim = params.get('hidden_dim')
    data_path = os.path.join(data_root, data_set)
    exp_path = 'result/dmpnn/'
    ...
    #model = MPNN_DDI(node_dim, edge_dim,n_iter=n_iter)
    model = MPNN_DDI(node_dim,edge_dim,hidden_dim=hidden_dim,n_iter=n_iter, kge_dim=kge_dim, rel_total=rel_total)
    ...
    for epoch in range(epochs):
        ...
        val_loss, val_acc, val_auroc, val_f1_score, val_precision, val_recall, val_ap,val_aupr  = val(model, criterion, val_loader, device)

        test_loss, test_acc, test_auroc, test_f1_score, test_precision, test_recall, test_ap, test_aupr = val(model,
                                                                                                               criterion,
                                                                                                               test_loader,
                                                                                                               device)

        msg = "epoch-%d, train_loss-%.4f, train_acc-%.4f, val_loss-%.4f, val_acc-%.4f,val_auroc-%.4f, val_f1_score-%.4f, val_prec-%.4f, val_rec-%.4f, val_ap-%.4f,val_aupr-%.4f, test_loss-%.4f, test_acc-%.4f, test_auroc-%.4f, test_f1_score-%.4f, test_prec-%.4f, test_rec-%.4f, test_ap-%.4f,test_aupr-%.4f" % (
            epoch, epoch_loss, epoch_acc, val_loss, val_acc, val_auroc, val_f1_score, val_precision, val_recall, val_ap,val_aupr,test_loss, test_acc, test_auroc, test_f1_score, test_precision, test_recall, test_ap,test_aupr)
        logger.info(msg)

        scheduler.step()

        if save_model:
            msg = "epoch-%d, train_loss-%.4f, train_acc-%.4f, val_loss-%.4f, val_acc-%.4f,test_loss-%.4f, test_acc-%.4f" % (epoch, epoch_loss, epoch_acc, val_loss, val_acc,test_loss,test_acc)
