...
def data_progress(dataset, triple_format='htr'):
    ...
    for file in file_list:
        triples_dict[file] = read_triple(os.path.join('transductive', dataset, file + '.txt'), triple_format=triple_format)
    ...
    if dataset == 'Hetionet':
        cases = kg.build_cases_for_large_graph(case_num=5, enclosing=False, hop=2)
    else:
...
dataset_list = ['codex-s', 'codex-m', 'codex-l', 'FB15k-237', 'WN18RR', 'NELL-995', 'YAGO3-10', 'DB100K', 'Hetionet',
                'ConceptNet', 'aristo-v4', 'ConceptNet']
