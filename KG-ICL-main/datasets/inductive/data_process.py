...
def data_progress(dataset, triple_format='htr'):
    ...
    for file in file_list:
        triples_dict[file] = read_triple(os.path.join('inductive', dataset, file + '.txt'), triple_format=triple_format)
...
mother_dataset = ['fb237', 'WN18RR', 'nell']

dataset_list = []
for dataset in mother_dataset:
    for v in range(4):
        dataset_list.append(dataset + '_v' + str(v+1))
        dataset_list.append(dataset + '_v' + str(v+1) + '_ind')
