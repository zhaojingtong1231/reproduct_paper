...
def data_progress(dataset, triple_format='htr'):
    ...
    for file in file_list:
        if file == 'valid':
            file_ = 'inference_validation'
        elif file == 'test':
            file_ = 'inference_test'
        else:
            file_ = file
        triples_dict[file] = read_triple(os.path.join('ILPC', dataset, file_ + '.txt'), triple_format=triple_format)
        # triples_dict[file] = read_triple(os.path.join(dataset, file + '.txt'))
    triples_dict['inference'] = read_triple(os.path.join('ILPC', dataset, 'inference' + '.txt'), triple_format=triple_format)
    ...
    kg_test = KG(triples_dict['inference'], len(entity2id_test), len(relation2id_test))
...
dataset_list = ['ILPC-large', 'ILPC-small']
