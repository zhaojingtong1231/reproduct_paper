...
def model_components():
    ...
    adapt_lr = 0.01
    meta_lr = 0.001
    model = FrontAndHead(input_dim=128, hid_dim=128, num_classes=2,  # 0 or 1
                         task_type="multi_label_classification",
                         token_num=10, cross_prune=0.1, inner_prune=0.3)

    # load pre-trained GNN
    gnn = GNN(input_dim=128, hid_dim=128, out_dim=128, gcn_layer_num=2, gnn_type='GCN')
    pre_train_path = './pre_trained_gnn/{}.SimGRACE.{}.pth'.format(dataname, 'GCN')
...
if __name__ == '__main__':
    ...
    meta_train_task_id_list = [6, 7, 8, 9]
    meta_test_task_id_list = [10, 11]

    pre_train_method = 'SimGRACE' # SimGRACE
    gnn_type = ['GCN']
    ...
    meta_train_maml(20, maml, gnn, lossfn, opt, meta_train_task_id_list,
                    dataname, adapt_steps=2, K_shot=5)

    # meta testing on target tasks
    adapt_steps_meta_test = 2  # 00  # 50
    meta_test_adam(meta_test_task_id_list, dataname, 5, seed, maml, gnn,
