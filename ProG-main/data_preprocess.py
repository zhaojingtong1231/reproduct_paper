...
from ProG.MRCGNN.data_preprocess import load_data
from ProG.MRCGNN.parms_setting import settings
import os
...
def load_tasks(meta_stage: str, task_pairs: list, dataname: str = None, K_shot=None, seed=0):
    ...
    :param K_shot:  default: None.
                    if K_shot is None, load the full small_data to train/test meta.
...
if __name__ == '__main__':
    ...
    # small_data = feature_reduce(small_data)
    # pk.dump(small_data, open('./dataset/{}/feature_reduced.small_data'.format(dataname), 'bw'))
    data_o, data_s, data_a, train_loader, val_loader, test_loader = load_data(args)
    data = pk.load(open('./Dataset/{}/feature_reduced.small_data'.format(dataname), 'br'))
    node_classes=10
    # small_data = data_o
